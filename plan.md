...
## 26. جراحة لوحة المتصدرين والتبويبات الجديدة (Leaderboard Hardening — 2026-08-22)

فحص شامل للواجهة (`public/leaderboard.js`) وخلفية الأنواع الجديدة (`functions/api/leaderboard.js`: ديوان العباد + عرش الإلهة) — الإصلاحات المنفذة:

| البند | الموقع | الإصلاح |
|---|---|---|
| XSS متبقٍ | `leaderboard.js:422,427,499` + الحرف الأول `:248,:496` | `esc()` على `star.name` (alt+نص) و`user.username` وحرف الأفاتار `initial` في اللوحتين — تغطي أسماء/مستخدمين مسجلين قبل بوابة charset |
| سباق الاستجابات | `loadLeaderboard` | عداد `requestGeneration`: الردود القديمة (تبديل سريع للفلاتر) تُهمل ولا ترسم تحت تبويب نشط آخر |
| هيكل عالق | فرع النجاح | `success:false` يرمي خطأ → صندوق خطأ مع زر «🔄 إعادة المحاولة» بدل skeletons للأبد |
| منسدلة مقصوصة | `populateCharactersDropdown` | `limit=2000` + TTL خمس دقائق للكاش + toast مرئي عند الفشل |
| تعادلات السلاسل | `backend:80` | حسم نهائي `, u.created_at_ms ASC` لفرع streaks (استقرار الترتيب بين التحديثات) |
| بطل السلطانة | `backend:231` | حسم `, u.username ASC` لتعادل topDevotee الكامل |
| اتساق عرش الإلهة | `backend:179-185` | مجاميع الولاء الجماعي تحسب `approved` غير المحذوفين فقط عبر `LEFT JOIN users pu` مع CASE guards — متسقة مع بوابة topDevotee؛ وتعليق واقٍ على MAX الثنائية داخل SUM (لا تُطوَّع لأحادية وإلا انهار الاستعلام كتجمية متداخلة) |
| شارات الأدوار | `frontend:254,:501` | توحيد إظهار شارة moderator («حَارِس المَعْبَد» / MOD) بجانب admin في اللوحتين |

### نتائج التحقق
- `vitest`: **39/39** (شاملة 3 اختبارات نقاط نهاية اللوحة ضد الخلفية المعدلة — أشكال الاستجابة لم تتغير).
- فحص نحوي: الواجهة والخلفية سليمتان; صفر مواضع interpolation خام متبقية لـstar.name/user.username.

### إضافات إصلاح بلاغين تشغيليين (2026-08-22)
| البلاغ | الجذر | الإصلاح |
|---|---|---|
| الخروج عند تحديث الصفحة | جولة الأمان جعلت كوكي `goooog_session` دائماً Secure وألغت التخزين المحلي → على http المحلي ترفض المتصفحات الكوكي فلا جلسة تُخزَّن | كوكي الاحتياط الاسمي يعكس بروتوكول الطلب (`isHttps`) في login/logout؛ `__Host-` يبقى Secure إجبارياً وفق المواصفة. الإنتاج https متطابق متصلّب، والمحلي يستعيد الاستمرارية |
| شريط الصور الأفقي لا يُسحب بالماوس | `overflow-x` أصلي فقط (لمس/تراكباد) مع شريط تمرير مخفي | سحب مؤشر كامل (`pointerdown/move/up` + عتبة 6px) على `.row-photos-strip`، ابتلاع النقرة اللاحقة للسحب عبر capture-phase، تعطيل smooth-scroll وسلوكيات hover أثناء السحب، منع drag الصور الأصلي، ومؤشرا grab/grabbing |

- `vitest`: **41/41** (+2 لعقد كوكي البروتوكول المزدوج في `tests/auth.test.js`).
